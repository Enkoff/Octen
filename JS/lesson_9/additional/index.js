...
usersList.forEach(obj => {
    ...
    email.classList.add('box');

    const addressUl = document.createElement('div');
    addressUl.innerText = 'ADDRESS:';
    addressUl.classList.add('box');

    for (let addressKey in obj.address) {
        if (addressKey !== 'geo') {
            const li = document.createElement('li');
            li.innerText = obj.address[addressKey];
            li.classList.add('box');
            addressUl.appendChild(li);
        } else {
            const geoUl = document.createElement('ul');
            geoUl.innerText = 'GEO:';
            geoUl.classList.add('box');

            for (let geoKey in obj.address.geo) {
                const geoLi = document.createElement('li');
                geoLi.innerText = `${obj.address.geo[geoKey]}`;
                geoLi.classList.add('box');
                geoUl.appendChild(geoLi);
            }
            addressUl.appendChild(geoUl);
        }
    }

    const phone = document.createElement('div');
    phone.innerText = `Phone: ${obj.phone}`;
    phone.classList.add('box');

    const website = document.createElement('div');
    website.innerText = `Web Suite: ${obj.website}`;
    website.classList.add('box');

    const companyUl = document.createElement('ul');
    companyUl.innerText = `COMPANY`;
    companyUl.classList.add('box');

    for (let companyKey in obj.company) {
        const companyLi = document.createElement('li');
        companyLi.innerText = `${obj.company[companyKey]}`;
        companyLi.classList.add('box');
    ...
    wrapper.appendChild(addressUl);
    wrapper.appendChild(phone);
    wrapper.appendChild(website);
    wrapper.appendChild(companyUl);
